...
def CVRPTW(
    ...
    if (type(edges) is not dict):
        if (edges == 'Complete_Euclidean'):
            ...
            for i in customerID:
                for j in customerID:
                    if (i != j):
                        edges[i, j] = math.sqrt((nodes[i]['loc'][0] - nodes[j]['loc'][0]) ** 2 + (nodes[i]['loc'][1] - nodes[j]['loc'][1]) ** 2)
            for i in customerID:
                edges[depotID, i] = math.sqrt((nodes[depotID]['loc'][0] - nodes[i]['loc'][0]) ** 2 + (nodes[depotID]['loc'][1] - nodes[i]['loc'][1]) ** 2)
                edges[i, depotID] = math.sqrt((nodes[i]['loc'][0] - nodes[depotID]['loc'][0]) ** 2 + (nodes[i]['loc'][1] - nodes[depotID]['loc'][1]) ** 2)
        else:
    ...
    while(canAddVarFlag):
        ...
        if (CVRPTW.status == GRB.status.OPTIMAL):
            ...
            fig, ax = vrpSolver.plotSeq(fig = fig, ax = ax, nodes = nodes, seq = subproblem['route'])
            fig.show()
